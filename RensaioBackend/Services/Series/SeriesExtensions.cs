...
using ImportEntity = RensaioBackend.Models.Database.ImportEntity;
using DbSeriesEntity = RensaioBackend.Models.Database.SeriesEntity;
...
public static class SeriesExtensions
{
    ...
    {
        bool isUnknown = string.IsNullOrEmpty(ImportProviderSnapshot.Provider)
                         || ImportProviderSnapshot.Provider == "Unknown";
    ...
    /// </summary>
    /// <param name="provider">The series provider whose chapters should be populated.</param>
    /// <param name="seriesBasePath">The base storage path for the series (e.g., Settings.StorageFolder + series.StoragePath).</param>
    public static void PopulateChapterPageCounts(this SeriesProviderEntity provider, string seriesBasePath)
    {
        foreach (var chapter in provider.Chapters.Where(c => !string.IsNullOrEmpty(c.Filename)))
        {
